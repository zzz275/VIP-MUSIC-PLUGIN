...
async def download_video(client, CallbackQuery):
    ...
    if current_time - last_Query_time < SPAM_WINDOW_SECONDS:
        ...
        await CallbackQuery.answer(
            "➻ الفـديـو بتاعك اتـحمل شـوف خــاص الـبوت.\n\n➥ ممـكن تحـمل تـاني بـعد 30 ثـانيه",
            show_alert=True,
        )
        return
    else:
    ...
    await CallbackQuery.answer("حـاضر يـقلبي بـتحمل اهـي.", show_alert=True)
    ...
    if not videoid:
        await pablo.edit(
            f"**يـسط {chutiya} مـش لاقـيها ع الـتيوب...**"
        )
        return
